...
export default function ParticleFusion() {
  ...
  useEffect(() => {
    ...
    // Capped below the display's own ratio on purpose. This layer is soft
    // bokeh, nothing with an edge to keep crisp, and it is the largest surface
    // on the page — at 2x it repaints four times the pixels of a 1x screen
    // every frame, which is where most of the site's idle cost was going.
    // Raise to 2 if the field ever needs to hold fine detail.
    ...
    const tick = (now: number, delta: number) => {
      ...
      for (let i = 0; i < stars.length; i++) {
        ...
        const clearing = CLEARING_DIM + (1 - CLEARING_DIM) * smoothstep(0.34, 1.08, dist(ex, ey));
