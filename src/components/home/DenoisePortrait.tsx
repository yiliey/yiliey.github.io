...
export default function DenoisePortrait({
  ...
  useEffect(() => {
    ...
    // Read back every frame while the noise lasts, so keep this one on the CPU:
    // without the hint each getImageData pulls the surface off the GPU first.
    const lctx = latent.getContext('2d', { willReadFrequently: true });
    ...
    image.onload = () => {
      let start = 0;
      // Match the CSS the real <img> underneath is laid out with
      // (`object-cover object-[32%_center]`). Drawing the whole photo stretched
      // into the square instead — which is what a bare drawImage(0,0,w,h) does —
      // showed a squashed, zoomed-out crop for the length of the animation, so
      // the portrait visibly jumped when the canvas came off at the end.
      const cover = Math.max(canvas.width / image.naturalWidth, canvas.height / image.naturalHeight);
      const fitW = image.naturalWidth * cover;
      const fitH = image.naturalHeight * cover;
      const fitX = (canvas.width - fitW) * 0.32;
      const fitY = (canvas.height - fitH) * 0.5;
      stop = onFrame(now => {
        if (!start) start = now;
        ...
        lctx.drawImage(image, fitX * k, fitY * k, fitW * k, fitH * k);
