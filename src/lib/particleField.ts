 ...
 * Plain 2D distance.
 *
 * `Math.hypot` is variadic and guards against intermediate overflow, which
 * makes it several times slower than the square root it wraps. Every distance
 * in this field is a handful of screen pixels, nowhere near the range where
 * that guard matters — and it is called once or twice per particle per frame,
 * a few hundred thousand times a second, so the difference is not academic.
 */
