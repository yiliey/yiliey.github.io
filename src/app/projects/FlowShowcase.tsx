...
const projects = [
  ...
  { title: "LoRA + Hybrid RAG for Banking Service", repo: "https://github.com/yiliey/bank-customer-service-llm", modes: [3, 4], summary: "Banking questions, answered from the documents that actually govern them.", contribution: "Built both halves: LoRA SFT and DPO post-training on Qwen2.5-3B, and a Parent\u2013Child hybrid retrieval layer over banking and regulatory PDFs.", problem: "Answer Chinese banking and regulatory questions with a domain-adapted model and a knowledge base that can be updated without retraining it.",
    details: [
      "Post-trained Qwen2.5-3B-Instruct with LoRA SFT on 3,999 banking dialogue examples through LLaMA-Factory, ablating LoRA rank, learning rate, and weight initialization to a verified rank-16 configuration.",
      "Lifted BLEU-4 from 6.12 to 27.61 and ROUGE-L from 17.81 to 39.04 against the base model, measured on 535 held-out test samples.",
      "Constructed a 2,000-pair DPO preference dataset scoring responses on domain professionalism, intent comprehension, and proactive clarification, then continued training the SFT adapter on it.",
      "Built a Parent\u2013Child hybrid RAG layer over banking and regulatory PDFs: FAISS dense recall merged with BM25 sparse recall, reranked by a BGE cross-encoder, then expanded from each matched child chunk back to its parent section so generation reads coherent context.",
    ],
